...
function CombatService:new(castService, config, commandBus, roleService, debuffService)
    ...
    self.stickDist    = config:get('AssistSettings.meleeStickDistance') or 10
    self.rotation =
    {
...
function CombatService:setStickPoint(point)
    self.stickPoint = point
end

function CombatService:setStickDist(dist)
    self.stickDist = tonumber(dist) or self.stickDist
end

function CombatService:reloadSet(set)
    local nukes = self:getNukesFromKey('Nukes.' .. set)
    if #nukes > 0 then
        self.rotation.spellRotation = nukes
    end
end
...
function CombatService:handleMed()
    ...
    -- never med during combat (pullers keep medding until they actually engage)
    if State.assist.targetId and not State.flags.isPuller then
    ...
    -- exit med mode when all resources the character has are full
    local manaFull = mq.TLO.Me.MaxMana() == 0 or mana >= medEnd
    local endFull  = mq.TLO.Me.MaxEndurance() == 0 or mq.TLO.Me.PctEndurance() >= medEnd
    if State.caster.medMode and manaFull and endFull then
...
function CombatService:assist()
    ...
    local assistSpawn = mq.TLO.Spawn('id ' .. State.assist.targetId)
    if not assistSpawn() or assistSpawn.Dead() then
        mq.cmd('/attack off')
        State:stopAssist()
        return
    end
    ...
    mq.cmdf('/stick %s %s moveback uw', self.stickPoint, self.stickDist)
    mq.delay(50)
    mq.cmd('/attack on')
end
...
function CombatService:canUse(entry)
    ...
    if entry.type == 'disc' then
        ...
        if activeDisc and activeDisc ~= '' then return false end
